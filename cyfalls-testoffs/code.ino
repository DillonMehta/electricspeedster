...
const double pwm_min = 40; // minimal PWM for movement
double Kpt = 0;            // proportional factor for turning
double left_angle = 79.5;  // approx. “normal” left turn angle
...
// subtract 1.75 seconds for every 50cm the end has, including the dist_diff
...
String buildCanBypass(double totalDist, double offset)
{
  ...
  double angleDeg = (angleRad * 180.0 / M_PI)+1; // degrees

  // distance along the diagonal to midpoint offset
  double leg = sqrt(pow(halfDist, 2) + pow(offset, 2));

  String seq = "";  
  seq += "L" + String(angleDeg, 1) + " "; 
  seq+="D200 ";                // turn toward cans
  seq += "F" + String(leg, 1) + " ";  // first diagonal
  seq+="D100 ";
  seq += "R" + String(2*angleDeg+5, 1) + " ";
  seq+="D200 "; 
  seq += "E";
  end_distance=leg;
  return seq;
...
void setup()
{
  ...
  String moveSeq = buildCanBypass(700, 80); // 500 cm A→B, cans 25 cm offset left

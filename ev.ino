...
void curveTurn(double turn_time, bool dir){
  ...
  while (true)
  {
    ...
    if(dir){
      double velocity_error_L = velocity_setpoint - vL();
      double velocity_error_R = 3/2 * velocity_setpoint - vR();
    }else{
      double velocity_error_L = 3/2* velocity_setpoint - vL();
      double velocity_error_R = velocity_setpoint - vR();
    }
    ...
    right_pwm = constrain(right_pwm, str_min, 400);
    if(dir){
      right_pwm -= kPt * (vL()/vR() - 2/3);
    }else{
      left_pwm -= kPt * (vR()/vL() - 2/3);
    }
    // Set motor speeds
    motors.setSpeeds(left_pwm, right_pwm);
  }
